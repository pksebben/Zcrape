...
impl MessageBuffer {
    ...
    pub fn from_file(file: &str) -> Result<MessageBuffer, Box<dyn Error>> {
        let st: String = fs::read_to_string(file)?;
        Ok(MessageBuffer::from_json_string(st))
    }
    pub fn from_json_string(json: String) -> MessageBuffer {
        ...
        for value in stream {
            ...
            if let serde_json::Value::Array(s) = messages {
                for message in s {
                    message_vec
                        .push(Message::from_sj_val(message).expect("could not construct message"));
                }
            } else {
    ...
    fn store(&self) -> Result<(), Box<MsgCacheError>> {
        ...
        fs::write(
            filename,
            serde_json::to_string(&self.messages).expect("could not serialize message buffer"),
        );
...
impl Message {
    ...
    fn from_sj_val(obj: serde_json::Value) -> serde_json::Result<Message> {
...
impl Cull for MessageBuffer {
    fn cull(&mut self, cullstring: &str) {
        self.messages
            .retain(|x: &Message| !x.content.contains(cullstring));
    }
    fn cull_list(&mut self, culls: Vec<&str>) {
        for cull in culls {
            self.messages
                .retain(|x: &Message| !x.content.contains(cull));
        }
    }
    fn keep(&mut self, keepstring: &str) {
        self.messages
            .retain(|x: &Message| x.content.contains(keepstring));
    }
    ...
    fn dedupe(&mut self) {
        self.messages
            .sort_unstable_by(|a, b| a.content.partial_cmp(&b.content).unwrap());
